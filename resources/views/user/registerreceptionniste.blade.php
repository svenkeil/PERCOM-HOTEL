...
<head>
  ...
  <title>REGISTER RECEPTIONNISTE| PERCOM-HOTEL</title>
...
<script>
  ...
  function login(info) {
    $.ajax({
        ...
        url: '{{route('process-recept-register')}}', //process-user-register est  le fichier php excute cote serveur
        ...
        success:function(response){
            ...
            if(response.status=='success'){
                ...
                setTimeout(()=>{
                    window.location.href="{{route('liste-receptionnistes')}}";
                }, 1000);
            }
