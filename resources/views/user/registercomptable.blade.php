...
<head>
  ...
  <title>REGISTER ACCOUNTING | PERCOM-HOTEL</title>
...
<script>
  ...
  function login(info) {
    $.ajax({
        ...
        url: '{{route('process-comptable-register')}}', //process-user-register est  le fichier php excute cote serveur
        ...
        success:function(response){
            ...
            if(response.status=='success'){
                ...
                setTimeout(()=>{
                    window.location.href="{{route('liste-comptables')}}";
                }, 1000);
            }
